...
import MaastCTS2.gnu.trove.list.array.TByteArrayList;
import MaastCTS2.gnu.trove.list.array.TDoubleArrayList;
import MaastCTS2.gnu.trove.list.array.TIntArrayList;
import MaastCTS2.gnu.trove.map.hash.TIntIntHashMap;
import MaastCTS2.heuristics.events.EventKnowledge;
import MaastCTS2.utils.OrderedIntPair;
import MaastCTS2.utils.Pathfinder;
...
public class KnowledgeBase {
	...
	private static final double DEFAULT_DIST_WEIGHT_AVATAR = 0.4;
	...
	public void init(StateObservationMulti stateObs){
	...
	public void addEventKnowledge(double previousScore, int previousNumEvents, Vector2d previousAvatarPos, 
									Vector2d previousAvatarOrientation, ACTIONS action, StateObservationMulti newState,
		...
		double scoreChange = newState.getGameScore(Agent.myID) - previousScore;
		boolean loss = (newState.isGameOver() && newState.getMultiGameWinner()[Agent.myID] == WINNER.PLAYER_LOSES);
		boolean win = (newState.isGameOver() && newState.getMultiGameWinner()[Agent.myID] == WINNER.PLAYER_WINS);
		...
		Vector2d newAvatarPos = newState.getAvatarPosition(Agent.myID);
		OrderedIntPair avatarCell = positionToCell(newAvatarPos);
		if(newState.getAvatarOrientation(Agent.myID).equals(previousAvatarOrientation) && !win && !loss){	// wins/losses make our orientation/position invalid
		...
		if(!(loss && playout)){
			...
			while(numNewEvents > 0 && it.hasNext()){
				...
				if(event.activeTypeId == newState.getAvatarType(Agent.otherID)){
					//System.out.println("opponent event!");
					continue;
				}
				...
				if(movementBlocked){
					...
					// so instead we'll pretend the object didn't disappear
				...
				eventKnowledge.get(spriteTypeIdx).updateObservation(scoreChange, movementBlocked, loss, disappearedObject);
				...
				if(!win && !loss){	// wins or losses make avatarCell invalid, so dont update average distance if win or loss
					...
					int interactionCounter = eventKnowledge.get(spriteTypeIdx).getObservationCount();
				...
				double error = eventKnowledge.get(spriteTypeIdx).getAverageDeltaScore() - currentWeight;
				...
				if(isAvatarCategory(typeCategories.getQuick(spriteTypeIdx))){
					//nextWeight = Math.max(nextWeight, 0.01);	// always want to stay a bit interested in the other avatar
				}
	...
	private double getInitialDistWeight(int category){
		if(isAvatarCategory(category)){
			return maxSingleStepScoreChange * DEFAULT_DIST_WEIGHT_AVATAR + Globals.smallNoise();
		}
		else if(isMovableCategory(category)){
	 ...
	 * @return
	 */
	public double knowledgeBasedEval(StateObservationMulti state){
		...
		OrderedIntPair avatarCell = positionToCell(state.getAvatarPosition(Agent.myID));
			...
			for (int y = 0; y < height; ++y){
				...
				for (int i = 0; i < numObservations; ++i){
					...
					if(isAvatarCategory(category)){
						...
						//continue;					// dont want to ignore avatars in 2-player games I think?
						
						if(cell.equals(avatarCell)){	// do want to ignore our own position though
							continue;
						}
					}
					if(isFromAvatarCategory(category)){
						//continue;		// dont want to ignore from-avatars in 2-player games I think?
					}
		...
		HashMap<Integer, Integer> resources = state.getAvatarResources(Agent.myID);
		...
		int hp = state.getAvatarHealthPoints(Agent.myID);
		...
		if(MIN_DANGEROUS_TYPES_EVAL != MAX_DANGEROUS_TYPES_EVAL){
		...
		// return total evaluation (which should be in [-0.80, 0.80])
		return eval + resourceEval + dangerousObservationsEval - obstacleTouchingMovablesPenalty - stuckObjectsPenalty;
	}
	...
	public void update(StateObservationMulti stateObs){
		...
		OrderedIntPair avatarCell = positionToCell(stateObs.getAvatarPosition(Agent.myID));
		if(isValidCell(avatarCell)){
			pheromones[avatarCell.first][avatarCell.second] += 1;
		}
	}
	
	public void updateRoot(StateObservationMulti rootState){
		...
		OrderedIntPair avatarCell = positionToCell(rootState.getAvatarPosition(Agent.myID));
			...
			for (int y = 0; y < height; ++y){
				...
				for (int i = 0; i < numObservations; ++i){
					...
					if(isAvatarCategory(category) || isFromAvatarCategory(category)){
						//continue;		// dont want to ignore these in 2-player games I think?
